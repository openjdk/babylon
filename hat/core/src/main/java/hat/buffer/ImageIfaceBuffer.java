...
public interface ImageIfaceBuffer<T extends ImageIfaceBuffer<?>> extends Buffer {
    ...
    default T syncFromRasterDataBuffer(DataBuffer dataBuffer) { // int[], byte[], short[], float[]
        switch (dataBuffer) {
            ...
            case DataBufferFloat arr ->
                    MemorySegment.copy(arr.getData(), 0, MappableIface.getMemorySegment(this), JAVA_FLOAT, 16L, arr.getData().length);
            default -> throw new IllegalStateException("Unexpected value: " + dataBuffer);
        }
        return (T)this;
    }

    default T syncFromRaster(BufferedImage bufferedImage) { // int[], byte[], short[], float[]
        return syncFromRasterDataBuffer(bufferedImage.getRaster().getDataBuffer());
    }

    @SuppressWarnings("unchecked")
    default T syncToRasterDataBuffer(DataBuffer dataBuffer) { // int[], byte[], short[], float[]
        switch (dataBuffer) {
            case DataBufferUShort arr ->
            ...
            case DataBufferFloat arr ->
                    MemorySegment.copy(MappableIface.getMemorySegment(this), JAVA_FLOAT, 16L, arr.getData(), 0, arr.getData().length);
            default -> throw new IllegalStateException("Unexpected value: " + dataBuffer);
        }
        return (T) this;
    }

    default T syncToRaster(BufferedImage bufferedImage) { // int[], byte[], short[], float[]
