...
public class OpBuilder2 {
     ...
     * @return Module Operation that consist of FuncOp that builds the model plus helper FuncOps.
     */
    public static ModuleOp createBuilderFunction(String name, Op op, Function<Block.Builder, Value> dialectFactoryF) {
        OpBuilder2 opBuilder = new OpBuilder2(dialectFactoryF);
        FuncOp builder = opBuilder.build(name, op);
        List<FuncOp> funcOps = new ArrayList<>(createSupportFunctions());
        funcOps.add(createExternTypeHelperFunc(opBuilder.registeredExternalizedTypes));
        funcOps.add(createTypeHelperFunc());
        funcOps.add(builder);
        return CoreOp.module(funcOps);
    }

    private static List<FuncOp> createSupportFunctions() {
        return List.of(
                ...
                func(OP_BUILDER_F_NAME + 1, OP_BUILDER_F_OVERRIDE_1).body(bb -> {
                    ...
                    b.op(return_(b.op(invoke(OP_FACTORY_CONSTRUCT,
                            ...
                            b.op(new_(ConstructorRef.constructor(EXTERNALIZED_OP_F_TYPE),
                                    args.get(0),
                                    args.get(1),
                                    b.op(funcCall(LIST_BUILDER_F_NAME, CoreType.functionType(J_U_LIST_VALUE), args.get(2))),
                                    b.op(funcCall(LIST_BUILDER_F_NAME, CoreType.functionType(J_U_LIST_BLOCK_REFERENCE), args.get(3))),
                                    args.get(4),
                                    b.op(funcCall(MAP_BUILDER_F_NAME, CoreType.functionType(J_U_MAP), args.get(5))),
                                    b.op(funcCall(LIST_BUILDER_F_NAME, CoreType.functionType(J_U_LIST_BODY_BUILDER), args.get(6)))))))));
                }),
                func(OP_BUILDER_F_NAME + 2, OP_BUILDER_F_OVERRIDE_2).body(bb -> {
                    Block.Builder b = bb.entryBlock();
                    List<Block.Parameter> args = b.parameters();
                    b.op(return_(b.op(invoke(BLOCK_BUILDER_OP,
                            args.get(0),
                            b.op(funcCall(OP_BUILDER_F_NAME + 1, OP_BUILDER_F_OVERRIDE_1,
                                    args.get(1),
                                    args.get(2),
                                    args.get(3),
                                    args.get(4),
                ...
                func(OP_BUILDER_F_NAME + 3, OP_BUILDER_F_OVERRIDE_3).body(bb -> {
                    Block.Builder b = bb.entryBlock();
                    List<Block.Parameter> args = b.parameters();
                    b.op(return_(b.op(funcCall(OP_BUILDER_F_NAME + 2, OP_BUILDER_F_OVERRIDE_2,
                            args.get(0),
                            args.get(1),
    ...
    OpBuilder2(Function<Block.Builder, Value> dialectFactoryF) {
    ...
    Value buildOp(Value blockBuilder,
        ...
        int i;
        if (override.equals(OP_BUILDER_F_OVERRIDE_1)) {
            i = 1;
        } else if (override.equals(OP_BUILDER_F_OVERRIDE_2)) {
            i = 2;
        } else {
            i = 3;
        }
        String funcName = OP_BUILDER_F_NAME + i;
        return builder.op(funcCall(funcName, override, args));
    }
    ...
    private static FuncOp createTypeHelperFunc() {
        /*
        ...
        */
        return func(TYPE_BUILDER_F_NAME, CoreType.functionType(type(TypeElement.class))).body(b -> {
    ...
    private static FuncOp createExternTypeHelperFunc(Map<ExternalizedTypeElement, List<Integer>> registeredExterTypes) {
        ...
        */
        return func(EXTER_TYPE_BUILDER_F_NAME, CoreType.functionType(type(ExternalizedTypeElement.class))).body(b -> {
            ...
            for (Map.Entry<ExternalizedTypeElement, List<Integer>> e : registeredExterTypes.entrySet()) {
                Body.Builder l = Body.Builder.of(b.parentBody(), CoreType.functionType(BOOLEAN));
                Block.Parameter target = l.entryBlock().parameter(INT);
                Integer typeIndex = e.getValue().getLast();
                Op.Result p = l.entryBlock().op(eq(target, l.entryBlock().op(constant(INT, typeIndex))));
                l.entryBlock().op(core_yield(p));

                Body.Builder expr = Body.Builder.of(b.parentBody(), CoreType.functionType(type(ExternalizedTypeElement.class)));
                List<Value> args = new ArrayList<>();
                ...
                for (int j = 0; j < e.getValue().size() - 1; j++) {
                    ...
                    Op.Result opr = expr.entryBlock().op(funcCall(EXTER_TYPE_BUILDER_F_NAME, CoreType.functionType(type(ExternalizedTypeElement.class)), index));
                    args.add(opr);
                }
                MethodRef mr;
                Op.Result type;
                ...
                swBodies.add(l);
                swBodies.add(expr);
                // if there is method for the number of argument we have
                // up to four
                // similar to buildList

                // if not, we combine in a list
            }
    ...
    private final SequencedMap<ExternalizedTypeElement, List<Integer>> registeredExternalizedTypes = new LinkedHashMap<>();
