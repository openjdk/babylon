...
public class CodeModelAttributeMapper implements AttributeMapper<CodeModelAttribute> {

    public static final CodeModelAttributeMapper INSTANCE = new CodeModelAttributeMapper();
    ...
    static Op readOp(BufReader buf, boolean terminal, Body.Builder ancestorBody, Block.Builder[] ancestorBodyBlocks, List<Value> allValues) {
        return ExtendedOp.FACTORY.constructOpOrFail(readExOp(buf, terminal, ancestorBody, ancestorBodyBlocks, allValues));
    }

    static ExternalizableOp.ExternalizedOp readExOp(BufReader buf, boolean terminal, Body.Builder ancestorBody, Block.Builder[] ancestorBodyBlocks, List<Value> allValues) {
    ...
    static void writeOp(BufWriter buf, Op op, Map<Value, Integer> valueMap) {
        // name
    ...
    static List<Value> readValues(BufReader buf, List<Value> allValues) {
        // number of values
    ...
    static void writeValues(BufWriter buf, List<Value> values, Map<Value, Integer> valueMap) {
        // number of values
    ...
    static List<Body.Builder> readNestedBodies(BufReader buf, Body.Builder ancestorBody, List<Value> allValues) {
        // number of bodies
    ...
    static void writeNestedBodies(BufWriter buf, List<Body> bodies, Map<Value, Integer> valueMap) {
        // number of bodies
    ...
    static void readBlocks(BufReader buf, Body.Builder bob, List<Value> allValues) {
        // number of blocks
    ...
    static void writeBlocks(BufWriter buf, List<Block> blocks, Map<Value, Integer> valueMap) {
        // number of blocks
    ...
    static void readBlockParameters(BufReader buf, Block.Builder bb, List<Value> allValues) {
    ...
    static void writeBlockParameters(BufWriter buf, List<Block.Parameter> parameters, Map<Value, Integer> valueMap) {
        // number of block parameters
    ...
    static void readOps(BufReader buf, Block.Builder bb, Block.Builder[] allBlocks, List<Value> allValues) {
        // number of ops
    ...
    static void writeOps(BufWriter buf, List<Op> ops, Map<Value, Integer> valueMap) {
        // number of ops
    ...
    static List<Block.Reference> readSuccessors(BufReader buf, Block.Builder[] ancestorBodyBlocks, List<Value> allValues) {
    ...
    static void writeSuccessors(BufWriter buf, List<Block.Reference> successors, Map<Value, Integer> valueMap) {
        // number of successors
    ...
    static FunctionType toFuncType(MethodTypeDesc mtd) {
        return FunctionType.functionType(JavaType.type(mtd.returnType()), mtd.parameterList().stream().map(JavaType::type).toList());
    }

    static MethodTypeDesc toMTD(FunctionType ftype) {
        return MethodTypeDesc.of(toCD(ftype.returnType()), ftype.parameterTypes().stream().map(CodeModelAttributeMapper::toCD).toList());
    }

    static ClassDesc toCD(TypeElement type) {
        return switch (type) {
